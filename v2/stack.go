...
// Stack manages nested v2 tea.Model overlays with composited tea.View output.
type Stack struct {
	entries      []stackEntry
	lastW, lastH int
	Adapter      ViewAdapter // if nil, [StringPipelineAdapter] is used
}
...
// StackDepth is an alias for Depth.
func (s *Stack) StackDepth() int {
	return s.Depth()
}
...
// MainReceivesKeys is false when keys and mouse should be routed only to the stack.
func (s *Stack) MainReceivesKeys() bool {
	return s == nil || len(s.entries) == 0
}

// MainReceivesMouseMsg matches the v1 overlay API name for easier porting.
func (s *Stack) MainReceivesMouseMsg() bool {
	return s.MainReceivesKeys()
}

// MainReceivesKeyMsg is an alias for [Stack.MainReceivesKeys].
func (s *Stack) MainReceivesKeyMsg() bool {
	return s.MainReceivesKeys()
}
...
// CompositeView builds the final tea.View for the given base layer string and viewport size.
...
// Update routes messages to stacked v2 models (same policy as v1 [overlay.OverlayStack]).
